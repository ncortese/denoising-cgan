...
class BatchLoader:
    ...
    def get_next_batch(self, batch_size):
        ...
        patch_list = [self.get_patch_i(i) for i in range(self.index, batch_end)]
        noise_list = [self.get_noise_i(i) for i in range(self.index, batch_end)]
        if batch_end == self.data_n:
            num_remaining = self.index + batch_size - self.data_n
            self.shuffle_data()
            patch_list = patch_list + [self.get_patch_i(i) for i in range(0, num_remaining)]
            noise_list = noise_list + [self.get_noise_i(i) for i in range(0, num_remaining)]
